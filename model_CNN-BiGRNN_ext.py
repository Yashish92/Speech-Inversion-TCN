...
Model : CNN-BiGRNN model with 6 TVs + source features as Targets
"""
...
from tensorflow.keras.layers import Bidirectional, LSTM, GRU, Dense, Input, Dropout, Concatenate, Masking, TimeDistributed, BatchNormalization, Conv1D
...
def blstm_model(sample_x):
    ...
    # # out = dense_2
    dense_2 = TimeDistributed(Dense(No_TVs, kernel_regularizer='l2'))(drp_3)
    out = tanh(dense_2)
...
def fine_tune(x_train, y_train, x_test, y_test, x_val, y_val):
    ...
    corr_avg, avg_corr, avg_corr_6TVs  = compute_corr_score(y_predict, y_test, No_TVs)

    # write a .txt with results and model params
    lines = ['Epochs:' + str(EPOCHS), 'Patience:' + str(PATIENCE), 'Batch size:' + str(BATCH_SIZE), 'Corr_Average:' + str(corr_avg), 'Corr_Average_all:' + str(avg_corr),
             'Corr_Average_6TVs:' + str(avg_corr_6TVs),'MSE:' + str(test_MSE), 'MAE:' + str(test_MAE)]
...
if __name__ == '__main__':
    ...
    if Feature == 'MFCCs':
        ...
        X_tr = np.load('data/Train_files/x_train_200_melspec_ap_per_pitch.npy')
        X_val = np.load('data/Train_files/x_val_200_melspec_ap_per_pitch.npy')
        X_te = np.load('data/Train_files/x_test_200_melspec_ap_per_pitch.npy')

        Y_tr = np.load('data/Train_files/y_train_200_melspec_ap_per_pitch.npy')
        Y_val = np.load('data/Train_files/y_val_200_melspec_ap_per_pitch.npy')
        Y_te = np.load('data/Train_files/y_test_200_melspec_ap_per_pitch.npy')
