...
Model : BiGRNN model with 6 TVs + source features as Targets
...
def blstm_model_small(sample_x):
    ...
    # out = dense_3
    dense_2 = TimeDistributed(Dense(No_TVs))(drp_3)
    out = tanh(dense_2)
...
def fine_tune(x_train, y_train, x_test, y_test, x_val, y_val):
    ...
    lines = ['Dataset:' + dataset, 'Normalization:' + norm_type ,'Epochs:' + str(EPOCHS), 'Patience:' + str(PATIENCE), 'Batch size:' + str(BATCH_SIZE), 'Corr_Average:' + str(corr_avg), 'Corr_Average_all:' + str(avg_corr),
...
if __name__ == '__main__':
    ...
    if norm_type == 'utterance-wise':
        ...
        if dataset == 'original_mfcc_plus':
            ...
            # For only MFCCs
            X_tr = np.load('data/Train_files/x_train_200_mfcc_ap_per_pitch.npy')

            # to test on clean speech
            X_val = np.load('data/Train_files/x_val_200_mfcc_ap_per_pitch.npy')
            X_te = np.load('data/Train_files/x_test_200_mfcc_ap_per_pitch.npy')
            ...
            Y_tr = np.load('data/Train_files/y_train_200_mfcc_ap_per_pitch.npy')

            # to test on clean speech
            Y_val = np.load('data/Train_files/y_val_200_mfcc_ap_per_pitch.npy')
            Y_te = np.load('data/Train_files/y_test_200_mfcc_ap_per_pitch.npy')
